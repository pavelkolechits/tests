...
export const CreateTestPage: FC = () => {
  ...
  const allTest = useTypedSelector(i => i.manageTestsReducer.allTest)
  ...
  const onClickHandler = (): void => {
    if(testName){
      dispatch({type: manageTestsActionTypes.GET_TEST, payload: {test: state, testName: value, id: getID()}})
      navigate('/start-test') 
      dispatch({type: createTestActionTypes.RESET_STATE})
      setValue('')
      setTestName(false)
      localStorage.setItem('allTests', JSON.stringify(allTest))
         localStorage.setItem('createTest', JSON.stringify(state))
     

    } else {
      dispatch({type: createTestActionTypes.CREATE_QUESTION, payload: {value, id: getID()}})
      setValue('')
    }
  ...
  return (
    <><Button className={style['create-button']} onClick={testNameHandler} variant='outline-success'>
      Create
    </Button>

    {testName&& <Button className={style['back-button']} onClick={cancelSave} variant="outline-secondary">Back</Button>}
